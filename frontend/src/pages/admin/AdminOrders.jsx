...
export default function AdminOrders() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-[#0f1111] mb-4">Orders ({orders.length})</h1>
      {orders.length === 0 ? (
        ...
        <div className="space-y-3">
          {orders.map((order) => (
            <div key={order._id} className="bg-white rounded-sm shadow-sm overflow-hidden">
              <div className="bg-[#f0f2f2] px-4 py-2.5 border-b border-[#d5d9d9] flex flex-wrap justify-between items-center gap-2 text-sm">
                <div className="flex gap-4 sm:gap-8">
                  ...
                  <div>
                    ...
                    <p className="text-xs">{order.user?.name} <span className="text-[#565959]">({order.user?.email})</span></p>
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select value={order.status} onChange={(e) => updateStatus(order._id, e.target.value)}
                    className="text-xs border border-[#888c8c] rounded px-2 py-1 bg-white focus:ring-2 focus:ring-[#007185] outline-none cursor-pointer">
                ...
                <div className="space-y-2">
                  {order.items.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 text-sm">
                      <img src={item.image} alt={item.name} className="w-10 h-10 object-contain bg-[#f7f7f7] rounded p-0.5" />
                      <span className="text-[#0f1111] flex-1">{item.name}</span>
                      <span className="text-[#565959]">x{item.quantity}</span>
